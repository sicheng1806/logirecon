...
use super::ReconsileColumn;
use crate::{DataFrame, Error, Result};

/// 对账器
///
/// 本质是DataFrame比对器, 通过设定各字段的对比方案  [ReconsileColumn] 来完成比对。
...
impl Reconsiler {
    ...
    pub fn new(
        ...
        let pk: Vec<_> = columns
            .iter()
            .filter(|(_, column)| **column == ReconsileColumn::PK)
            .map(|(name, _)| name.to_string())
            .collect();
        if pk.len() != 1 {
            return Err(Error::Impl(format!(
                "目前只支持单主键的对比器, 目前主键数量: {}",
                pk.len()
            )));
        }
        ...
        if right_names != names {
            return Err(Error::Impl("右数据框不匹配列名".into()));
        }
        Ok(Self {
            columns,
            left,
    ...
    /// 生成宽格式对比结果
    pub fn build_result(mut self) -> Result<Self> {
        use polars::prelude::*;
        //
        ...
        let diff_exprs: Vec<_> = self
            ...
            .map(|(name, column)| {
                ...
                let expr = match column {
                    ReconsileColumn::Exact => when(col(left_field).eq(col(right_field)))
                        .then(lit(NULL))
                        .otherwise(
                            format_str("{} != {}", [col(left_field), col(right_field)]).unwrap(),
                        ),
                    ReconsileColumn::Numberic(atol) => when(
                        col(left_field)
                            .sub(col(right_field))
                            .abs()
                            .lt_eq(lit(*atol)),
    ...
    pub fn get_width_result(&self) -> Result<DataFrame> {
        if let Some(result) = self.result.clone() {
            ...
            Err(Error::Impl("实现错误，需要先运行 build_result 方法".into()))
        }
    }
    ...
    fn primary_key(&self) -> &String {
        self.columns
            .iter()
            .find(|(_, column)| **column == ReconsileColumn::PK)
            .unwrap()
            .0
    }

    fn iter_reconsiled(&self) -> impl Iterator<Item = (&String, &ReconsileColumn)> {
        self.columns.iter().filter(|(_, column)| {
            **column != ReconsileColumn::PK && **column != ReconsileColumn::None
        })
    }
}
...
mod tests {
    use super::*;
    use crate::reconsile::ReconsileOption;
